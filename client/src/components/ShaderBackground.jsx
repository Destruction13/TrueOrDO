/**
 * ShaderBackground - Фоновый WebGL шейдер на основе CPPN (Compositional Pattern Producing Network)
 * 
 * Использование:
 *   import ShaderBackground from './components/ShaderBackground';
 *   <ShaderBackground />
 * 
 * Флаг отключения: установите ENABLE_SHADER_BG = false ниже или через env VITE_ENABLE_SHADER_BG=false
 * 
 * Требования:
 *   - three
 *   - @react-three/fiber
...
// ===================== ФЛАГ ВКЛЮЧЕНИЯ =====================
// Установите false чтобы отключить шейдер
const ENABLE_SHADER_BG = import.meta.env.VITE_ENABLE_SHADER_BG !== 'false';
...
function ShaderBackground() {
  ...
  // Если шейдер отключён флагом
  if (!ENABLE_SHADER_BG) {
    return null;
  }

  // Graceful fallback если WebGL недоступен
  if (!webglSupported) {
    return (
      <div 
        style={{
  ...
  const camera = { position: [0, 0, 1], fov: 75, near: 0.1, far: 1000 };
    ...
    <div style={containerStyle} aria-hidden="true">
      <div style={canvasContainerStyle}>
        <Canvas
          ...
          dpr={Math.min(window.devicePixelRatio, 2)} // Ограничиваем DPR до 2
          style={{ width: '100%', height: '100%' }}
        >
          <ShaderPlane />
        </Canvas>
      </div>
      {/* Градиентный оверлей для лучшей читаемости UI */}
